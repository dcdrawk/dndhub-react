...
class Button extends Component {
  ...
  componentDidMount() {
    ...
    // var buttons = this.button.querySelectorAll('button.paper-button');
    // console.log(buttons);
  ...
  render() {
    return (
      <button ref={(c) => {this.button = c} }
              className={`button paper-button ${this.props.raised ? 'raised' : 'flat'} ${this.props.primary ? 'primary' : ''}`}>{this.props.children}</button>
    );
